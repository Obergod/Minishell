/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   test_simple.c                                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: ufalzone <ufalzone@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/21 18:10:48 by ufalzone          #+#    #+#             */
/*   Updated: 2025/03/21 18:03:27 by ufalzone         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/parsing.h"
#include "../../includes/token.h"
#include "../../includes/garbage_collector.h"
#include <stdio.h>

// Fonction pour créer manuellement des tokens pour tester
t_token *create_test_tokens(t_minishell *minishell, char *input)
{
	t_token *head = NULL;
	t_token *current = NULL;
	t_token *new_token = NULL;
	char **words = NULL;
	int i = 0;

	// Si input est fourni, le diviser en mots
	if (input)
	{
		words = gc_split(input, ' ', minishell->gc);
		if (!words)
			return NULL;

		// Parcourir chaque mot et créer un token approprié
		while (words[i])
		{
			new_token = gc_malloc(sizeof(t_token), minishell->gc);
			new_token->str = gc_strdup(words[i], minishell->gc);

			// Détecter les types de tokens
			if (ft_strcmp(words[i], "|") == 0)
				new_token->type = T_PIPE;
			else if (ft_strcmp(words[i], "<") == 0 || ft_strcmp(words[i], ">") == 0 ||
					ft_strcmp(words[i], "<<") == 0 || ft_strcmp(words[i], ">>") == 0)
				new_token->type = T_REDIR;
			else if (ft_strcmp(words[i], "&&") == 0 || ft_strcmp(words[i], "||") == 0 ||
					ft_strcmp(words[i], "(") == 0 || ft_strcmp(words[i], ")") == 0)
				new_token->type = T_LOGIC;
			else
				new_token->type = T_WORD;

			new_token->state = NORMAL;
			new_token->next = NULL;

			if (!head)
				head = new_token;
			else
				current->next = new_token;

			current = new_token;
			i++;
		}
	}
	else
	{
		// Exemple par défaut: "ls -la | grep test"
		// Token 1: "ls"
		new_token = gc_malloc(sizeof(t_token), minishell->gc);
		new_token->str = gc_strdup("ls", minishell->gc);
		new_token->type = T_WORD;
		new_token->state = NORMAL;
		new_token->next = NULL;
		head = new_token;
		current = new_token;

		// Token 2: "-la"
		new_token = gc_malloc(sizeof(t_token), minishell->gc);
		new_token->str = gc_strdup("-la", minishell->gc);
		new_token->type = T_WORD;
		new_token->state = NORMAL;
		new_token->next = NULL;
		current->next = new_token;
		current = new_token;

		// Token 3: "|"
		new_token = gc_malloc(sizeof(t_token), minishell->gc);
		new_token->str = gc_strdup("|", minishell->gc);
		new_token->type = T_PIPE;
		new_token->state = NORMAL;
		new_token->next = NULL;
		current->next = new_token;
		current = new_token;

		// Token 4: "grep"
		new_token = gc_malloc(sizeof(t_token), minishell->gc);
		new_token->str = gc_strdup("grep", minishell->gc);
		new_token->type = T_WORD;
		new_token->state = NORMAL;
		new_token->next = NULL;
		current->next = new_token;
		current = new_token;

		// Token 5: "test"
		new_token = gc_malloc(sizeof(t_token), minishell->gc);
		new_token->str = gc_strdup("test", minishell->gc);
		new_token->type = T_WORD;
		new_token->state = NORMAL;
		new_token->next = NULL;
		current->next = new_token;
	}

	return head;
}

// Fonction pour débugger et afficher le contenu d'une liste chaînée de commandes
void debug_cmd_list(t_cmd *cmd)
{
	int cmd_count = 0;

	while (cmd)
	{
		cmd_count++;
		printf("\n---- Commande #%d ----\n", cmd_count);
		printf("Command raw: %s\n", cmd->command_raw ? cmd->command_raw : "NULL");

		printf("Arguments: ");
		if (cmd->command)
		{
			for (int i = 0; cmd->command[i] != NULL; i++)
			{
				printf("[%s] ", cmd->command[i]);
			}
		}
		else
		{
			printf("NULL");
		}
		printf("\n");

		printf("Arg count: %zu\n", cmd->_arg_count);
		printf("Arg capacity: %zu\n", cmd->_arg_capacity);
		printf("Infile: %s\n", cmd->infile ? cmd->infile : "NULL");
		printf("Outfile: %s\n", cmd->outfile ? cmd->outfile : "NULL");
		printf("Append: %d\n", cmd->append);
		printf("Logic operator: %d\n", cmd->logic_operator_type);

		printf("Logic operator type: ");
		switch (cmd->logic_operator_type)
		{
			case NONE: printf("NONE\n"); break;
			case AND: printf("AND\n"); break;
			case OR: printf("OR\n"); break;
			case OPEN_PARENTHESIS: printf("OPEN_PARENTHESIS\n"); break;
			case CLOSE_PARENTHESIS: printf("CLOSE_PARENTHESIS\n"); break;
			default: printf("UNKNOWN\n");
		}

		printf("Heredoc: %s\n", cmd->heredoc ? cmd->heredoc : "NULL");

		cmd = cmd->next;
	}
}

int main(int argc, char **argv)
{
	t_minishell minishell;
	t_token *tokens;
	t_cmd *commands;
	char *input = NULL;

	// Récupérer l'input depuis les arguments si disponible
	if (argc > 1)
		input = argv[1];

	// Initialisation du garbage collector
	minishell.gc = gc_init();
	minishell.env = NULL;
	minishell.cmd = NULL;

	// Création des tokens de test
	tokens = create_test_tokens(&minishell, input);
	if (!tokens)
	{
		printf("Erreur: échec de la création des tokens\n");
		gc_destroy(minishell.gc);
		return 1;
	}

	// Affichage des tokens pour vérification
	printf("==== Tokens ====\n");
	t_token *current = tokens;
	int token_count = 0;
	while (current)
	{
		token_count++;
		printf("Token #%d: '%s' (Type: %d, State: %d)\n",
			token_count, current->str, current->type, current->state);
		current = current->next;
	}

	// Appel à la fonction parsing
	commands = parsing(tokens, &minishell);

	// Vérification du résultat
	if (!commands)
	{
		printf("\nLa fonction parsing a retourné NULL. Vérifiez les erreurs.\n");
		gc_destroy(minishell.gc);
		return 1;
	}

	// Affichage des commandes parsées
	printf("\n==== Commandes parsées ====\n");
	debug_cmd_list(commands);

	// Libération de la mémoire
	gc_destroy(minishell.gc);
	return 0;
}
