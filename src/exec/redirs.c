...
/*   Updated: 2025/05/14 20:16:54 by ufalzone         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	apply_redirections(int *fd_in, int *fd_out, t_ast_node *node)
{
	...
	//	else if (node->cmd->logic_operator_type == CLOSE_PARENTHESIS && dup2(*fd_in, STDIN_FILENO) == -1)
	//		return (perror("minishell: dup2"), 1);
		close(*fd_in);
		*fd_in = -1;
	}
	if (*fd_out != -1)
	{
		if (dup2(*fd_out, STDOUT_FILENO) == -1)
			return (perror("minishell: dup2"), 1);
//		else if (node->subshell == 1 && dup2(*fd_out, STDOUT_FILENO) == -1)
//			return (perror("minishell: dup2"), 1);
		close(*fd_out);
		*fd_out = -1;
	}
